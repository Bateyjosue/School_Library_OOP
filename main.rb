...
def main
  ...
  loop do
    ...
    when 7 then 
      ...
      $obj_book = {}
      $obj_rental = {}
      app.persons.map {|x| $obj_person[:id] = x.id; $obj_person[:name] = x.name; $obj_person[:age] = x.age}
      ...
      app.books.map {|x| $obj_book[:title] = x.title; $obj_book[:author] = x.author}
      ...
      app.rentals.map {|x| $obj_rental[:date] = x.date; 
      $obj_rental[:person_id] = x.person.id; $obj_rental[:person_name] =x.person.name; $obj_rental[:person_age] = x.person.age;
      $obj_rental[:book_title] = x.book.title; $obj_rental[:book_author] = x.book.author;
    }
